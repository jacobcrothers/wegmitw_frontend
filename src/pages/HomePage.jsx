...
import { getData } from "../slices/homeSlice";
...
const Home = () => {
  ...
  let cnt = 1;
  const buyerTableRows = useMemo(() => buyerData.map(row => {
    return (
      <ul className="list-unstyled mb-0 w-100">
        <li className="mb-0 d-inline-block">
          <span>{cnt++}</span>
          <figure className="mb-0 d-inline-block">
            <img src={row.logo} className="img-fluid nft-avatar" alt="vps-icon1" />
          </figure>
          {row.name}
        </li>
        <li className="mb-0 d-inline-block text-center">
          ${Number((row.marketCap).toFixed(2))}
        </li>
        <li className="mb-0 d-inline-block text-center">
          {row.holderNum}
          <br />
          <span className="p-0 color-68cf29">+{Number((row.holderNumPercent * 100).toFixed(2))}%</span>
        </li>
        <li className="mb-0 d-inline-block text-center">
          {row.buyerNum24h}
        </li>
  ...
  const nftDropsComp = useMemo(() => dropsData.map(row => {
    return (
        ...
        <div className="service-box-item bg-3b3363 w-100 float-left">
          <div className="drop-img w-100 float-left">
            <figure className="mb-0 float-left">
              <img src={row.logo} alt="cat-icon" className="img-fluid nft-avatar" />
            </figure>
          ...
          <div className="w-100 float-left price-con">
            ...
            <ul className="list-unstyled w-100 float-left">
              ...
              <li className="d-inline-block">
                <span className="pl-0">{row.total}</span>
              </li>
            </ul>
            <ul className="list-unstyled w-100 float-left">
              <li className="d-inline-block">
